...
void sendSensorData() {
  ...
  if (t > TEMP_MAX && (now - lastHighTempNotification >= NOTIFICATION_INTERVAL)) {
    Blynk.logEvent("sec_high_temp", "[Hive 2] The beehive temperature is too high, which may stress the bees. Ensure the hive is shaded and well-ventilated. Consider relocating it or adding a cooling mechanism to maintain an optimal environment.");
    ...
    Blynk.logEvent("sec_low_temp", "[Hive 2] The beehive temperature has dropped below the ideal range. Insulate the hive or provide external heating to protect the colony, especially during colder seasons.");
  ...
  if (h > HUMIDITY_MAX && (now - lastHighHumidNotification >= NOTIFICATION_INTERVAL)) {
    Blynk.logEvent("sec_high_humid", "[Hive 2] High humidity levels detected, which could encourage mold growth and harm the colony. Improve ventilation and check for water pooling near the hive.");
    ...
    Blynk.logEvent("sec_low_humid", "[Hive 2] Humidity levels are too low, potentially leading to dehydration of bees or poor brood development. Consider placing a water source near the hive or adjusting hive insulation.");
  ...
  if (t >= TEMP_MIN && t <= TEMP_MAX && h >= HUMIDITY_MIN && h <= HUMIDITY_MAX && (now - lastIdealConditionNotification >= NOTIFICATION_INTERVAL)) {
    Blynk.logEvent("sec_good_temphum", "[Hive 2] Good news! The hive's temperature is within the ideal range of 32–37°C, and humidity is stable between 55–65%. No action needed!");
